...
module.exports = {
  ...
  makePayment: async (req, res) => {
    ...
    try {
      const { amount, method, transactionId, academicYear } = req.body;
      ...
      if (!shiftFee) {
        shiftFee = await ShiftFee.create({
          student: studentId,
          shift: student.shift,
          fee: 0, // Will be updated later if needed
      ...
      const payment = await Payment.create({
        ...
        method,
        transactionId,
        verifiedBy: ["super-admin", "sub-admin"].includes(req.user.role)
          ? req.user.id
          : null,
        academicYear: shiftFee.academicYear,
      });

      // Update shift fee with payment reference
      ...
      //       await verifyWithPaymentGateway(payment, session); // Custom implementation
      //     } else {
      ...
      // sendPaymentReceipt(student.email, payment._id);
      res.status(201).json({
        success: true,
        data: {
          ...
          receiptUrl: `/receipts/${payment._id}`,
        },
      });
    } catch (error) {
